...
describe('walkthrough-nvim.model.schema', function()
  it('exposes the expected enums', function()
    ...
    assert.equals('has_many', schema.RELATIONSHIP_KIND.HAS_MANY)
    assert.equals('has_one', schema.RELATIONSHIP_KIND.HAS_ONE)
    assert.equals('belongs_to', schema.RELATIONSHIP_KIND.BELONGS_TO)
    assert.equals('many_to_many', schema.RELATIONSHIP_KIND.MANY_TO_MANY)
  end)
  ...
  it('accepts data_entities, ER relationships between them, and reads/writes from components into them', function()
    local ok, errors = schema.validate(load_fixture('valid-data-entities.json'))
    assert.is_true(ok, table.concat(errors, '; '))
    assert.same({}, errors)
  end)

  it('rejects a relationship pointing at an unknown data entity id', function()
    local ok, errors = schema.validate(load_fixture('invalid-dangling-data-entity.json'))
    assert.is_false(ok)
